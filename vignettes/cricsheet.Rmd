...
Additionally, when I originally worked on this plot of [Healy vs. the world](https://twitter.com/jacquietran/status/1448534315202080773), it was mid-October 2021 and the first ball of #WBBL07 had yet to be bowled. Now, we are reproducing the chart as we are part-way through the current season, so the code below also omits data from any matches played so far in this 7th season.
...
p <- p + geom_line(size = 2, colour = paletteer_d("beyonce::X25")[5])
p <- p + gghighlight(
  is_healy > 0,
  label_key = striker,
  label_params = aes(
...
Here’s the code for building [the plot I tweeted out](https://twitter.com/jacquietran/status/1449247570656522241). I decided to build one plot per player of interest and then “quilt” the plots together using the `patchwork` package (https://github.com/thomasp85/patchwork). I’m sure you could achieve the same / similar result using `ggplot2::facet_wrap()` but I just love using `patchwork` ...
...
plot_features <- list(
  ...
  theme(
    ...
    plot.subtitle = element_text(
      size = 14, family = "staat", margin=margin(0,0,15,0)),
    plot.caption = element_markdown(
      size = 10, margin=margin(15,0,0,0)),
    axis.text = element_text(size = 10, colour = "#FFFFFF"),
    legend.position = "none",
...
p <- p + geom_boxplot(
  fill = "#FFFFFF", colour = "#FFFFFF", size = .5, alpha = 0.25,
...
tweetrmd::tweet_screenshot("https://twitter.com/alyssastweeting/status/1458000401291120641")
```
